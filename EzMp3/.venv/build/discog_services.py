...
def search_discogs(access_token, search_params):
    ...
    for release in results["results"]:
        "title, contributing artist, album artist, album, year, and genre"
        print(f'\n\t== discogs-id {release["id"]} ==')
        print(f'\tTitle\t\t: {release.get("title", "Unknown")}')
        print(f'\tContributing Artist\t: {", ".join(release.get("artist", ["Unknown"]))}')
        print(f'\tAlbum Artist\t: {", ".join(release.get("album_artist", ["Unknown"]))}')
        print(f'\tAlbum\t\t: {release.get("title", "Unknown")}')
        print(f'\tYear\t\t: {release.get("year", "Unknown")}')
        print(f'\tGenre\t\t: {", ".join(release.get("genre", ["Unknown"]))}')
