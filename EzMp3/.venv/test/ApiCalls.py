...
d = discogs_client.Client('ezmp3/1.0')

# OAuth2 authentication process
try:
    request_token, request_secret, authorize_url = d.get_authorize_url()
    print(f'Please visit {authorize_url} to authorize access and get the verification code.')

    # Prompt the user to input the verification code they received after authorizing
    verification_code = input('Enter the verification code: ')

    # Obtain access token
    access_token, access_secret = d.get_access_token(verification_code)
    print('Access Token:', access_token)
    print('Access Secret:', access_secret)

except DiscogsAPIError as e:
    print(f"Error during OAuth2 process: {e}")

# Search using the authenticated client
try:
    results = d.search('Nirvana', type='artist')
